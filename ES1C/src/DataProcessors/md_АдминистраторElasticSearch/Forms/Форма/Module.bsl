...
&НаКлиенте
Процедура НастройкиРазмещенияОтчетаСравенениеВерсий(Команда)
	
	ОткрытьФорму("РегистрСведений.md_НастройкиРазмещенияОтчетаСравнениеВерсий.ФормаСписка");
...
Процедура ВсегоВерсийПриИзменении(Элемент)
	...
	Если НЕ ВсегоВерсий = ВсегоВерсийСтарое Тогда
		
		ИзменитьВерсий = ВсегоВерсий - ?(ВсегоВерсийСтарое = Неопределено, 0, ВсегоВерсийСтарое);
		ИзменитьКоличествоВерсийНаСервере(ИзменитьВерсий);
		ВсегоВерсийСтарое = ВсегоВерсий;
...
Процедура ИзменитьКоличествоВерсийНаСервере(ИзменитьВерсий)
	...
	НачальноеЗначение = ?(ИзменитьВерсий > 0, ВсегоВерсий-ИзменитьВерсий, ВсегоВерсий);
	КонечноеЗначение = ?(ИзменитьВерсий > 0, ВсегоВерсий, ВсегоВерсий - ИзменитьВерсий);
...
&НаСервере
Процедура ЗаполнитьСписокМетодовHTTP()
      
    Элементы.МетодHTTP.СписокВыбора.ЗагрузитьЗначения(ЗаполнитьМассивМетодовHTTP());
    
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЗаполнитьМассивМетодовHTTP()
    
 	Возврат md_ОбщегоНазначенияElasticsearch.СписокМетодовHTTP(); 
    
КонецФункции
 
&НаСервере
Функция ЗаполнитьЗаголовкиПоТаблице(ТаблицаЗаголовков)

	Заголовки = Новый Соответствие();
	
	Для Каждого ОписаниеЗаголовка ИЗ ТаблицаЗаголовков Цикл
		
		Заголовки.Вставить(ОписаниеЗаголовка.Заголовок, ОписаниеЗаголовка.Значение);
...
&НаСервере
Процедура ЗаполнитьЗаголововкиПоСоответствию(ТаблицаЗаголовков, Заголовки)

	ТаблицаЗаголовков.Очистить();
	
	Для Каждого КлючИЗначение ИЗ Заголовки Цикл
		
		СтруктраЗаполнения = Новый Структура("Заголовок, Значение", КлючИЗначение.Ключ, КлючИЗначение.Значение);
		ЗаполнитьЗначенияСвойств(ТаблицаЗаголовков.Добавить(), СтруктраЗаполнения);
...
Процедура ВыполнитьЗапросНаСервере()
    ...
    Заголовки = ЗаполнитьЗаголовкиПоТаблице(ЗаголовкиЗапроса);
    
    СтруктураОтправкиПакета = Новый Структура;  
    СтруктураОтправкиПакета.Вставить("Заголовки", Заголовки);
    ...
    ЗаполнитьЗаголововкиПоСоответствию(ЗаголовкиОтвета, ОтветES.ЗаголовкиОтвета);
...
Процедура ИнициализироватьНачальныеЗначенияФормы()
    ...
    ВсегоВерсий = ОтображаемоеКоличествоВерсийПоУмолчанию;
    ДатаОтбора = ТекущаяДатаСеанса();
    ВидСравнения = "Меньше или равно";
...
Процедура ОбработатьПостроениеДреваВерсийНаСервере()
    ...
    Если НЕ ЗначениеЗаполнено(ВсегоВерсийСтарое) 
        ИЛИ НЕ ВсегоВерсий = ВсегоВерсийСтарое Тогда
		
		ИзменитьВерсий = ВсегоВерсий - ?(НЕ ЗначениеЗаполнено(ВсегоВерсийСтарое), 0, ВсегоВерсийСтарое);
		ИзменитьКоличествоВерсийНаСервере(ИзменитьВерсий);
		ВсегоВерсийСтарое = ВсегоВерсий;
...
Процедура ОбработатьДействияПриИзмененииНастроекИнстанса(Действия)
    ...
    Для каждого Действие Из Действия Цикл
        ...
        Если Действие = "ЗаполнитьИндексы" Тогда
             ...
             ПолучитьИндексыESПоСерверуИСсылке(СерверElasticsearch, СсылкаНаОбъект));
...
Функция СформироватьПараметрыСравненияВерсий()
    ...
    ПараметрыСравнения.Вставить("ИндексES", ИндексES);
    ...
    ПараметрыСравнения.Вставить("ВсегоВерсий", ВсегоВерсий);
    ПараметрыСравнения.Вставить("СсылкаНаОбъект", СсылкаНаОбъект);
    
    АдресСервера = md_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СерверElasticsearch, "АдресСервера");
    Порт = md_ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СерверElasticsearch, "Порт");
...
Процедура ПоказатьФормуДлительнойОперации()
	...
	ПараметрыФормы.Вставить("ТекстЗаголовка", СформироватьПредставлениеОбъекта(СсылкаНаОбъект));
