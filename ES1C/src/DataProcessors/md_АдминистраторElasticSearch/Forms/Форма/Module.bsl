...
Функция ЗаполнитьДанныеОбъектаПоСтруктуре(ДанныеОбъектаВСтруктуре)
    ...
    Для каждого Версия Из ДанныеОбъектаВСтруктуре.КраткиеХарактеристикиВерсий Цикл
        ...
        ЗаписьXML.ЗаписатьБезОбработки(ПредставлениеВерсии); // Киллер фича - string builder    
        
    КонецЦикла; 
    
    ОписаниеВерсий = ЗаписьXML.Закрыть();
...
Процедура СформироватьДанныеСравненияВерсийНаКлиенте()
	...
	ПродолжитьФормированиеДанныехСравнения(Результат);
		              
КонецПроцедуры

&НаКлиенте
Процедура ПродолжитьФормированиеДанныехСравнения(Результат)
	
	Перем Статус;
	
	Если Результат.Свойство("Статус", Статус) И Статус = "ВыполненоУспешно" Тогда
...
Процедура ПродолжитьФормированиеДанныхСравнения(Результат)
    ...
    ЭтотОбъект.ПараметрыФоновогоЗадания.Добавить(Результат.Детали.ИдентификаторЗадания);
    ЭтотОбъект.ПараметрыФоновогоЗадания.Добавить(Результат.Детали.АдресХранилища);
