...
Процедура ПроиндексироватьДокументВElasticsearchBulkApi(ПараметрыОтправкиПакета, СообщениеОтправки)
	...
	ВыполнитьЗапросКElasticSearch(СтруктураОтправкиПакета);
...
Функция ПроиндексироватьДокументВElasticsearch(ПараметрыОтправкиПакета, СообщениеОтправки)
	...
	Возврат ВыполнитьЗапросКElasticSearch(СтруктураОтправкиПакета);
...
Функция ПроверкаНаПоддерживаемуюВерсию(ПараметрыОтправкиПакета)
	...
	ОтветES = ВыполнитьЗапросКElasticSearch(СтруктураОтправкиПакета);
...
Функция ВыполнитьЗапросКElasticSearch(СтруктураОтправкиПакета) Экспорт
	...
	HTTPСоединение = Новый HTTPСоединение(СтруктураОтправкиПакета.АдресСервера, СтруктураОтправкиПакета.Порт);
	
	HTTPЗапрос = Новый HTTPЗапрос;
	ЗаполнитьЗначенияСвойств(HTTPЗапрос, СтруктураОтправкиПакета, "Заголовки,АдресРесурса");
	ЗапросБезТела = НЕ СтруктураОтправкиПакета.Свойство("ТелоЗапроса"); 
    
    Если НЕ ЗапросБезТела Тогда
        
		HTTPЗапрос.УстановитьТелоИзСтроки(СтруктураОтправкиПакета.ТелоЗапроса, "UTF-8", 
    ...
    ЗапросУспешен = Истина;
    Попытка
        
        HTTPОтвет = HTTPСоединение.ВызватьHTTPМетод(СтруктураОтправкиПакета.HTTPМетод, HTTPЗапрос);
        
    Исключение
        
        ЗапросУспешен = Ложь;
        ТелоОтвета = ОписаниеОшибкиВТелоОтвета(ОписаниеОшибки());
               
    КонецПопытки;
    
	СтруктураЖурналаСообщений.Вставить("Длительность", ТекущаяУниверсальнаяДатаВМиллисекундах() - ДатаНачалаЗапроса);
    
    Если ЗапросУспешен Тогда
    ...
    ХранилищеЗапроса = Новый ХранилищеЗначения(?(ЗапросБезТела, "", СтруктураОтправкиПакета.ТелоЗапроса),
    МаксимальноеСжатие);
	...
	СтруктураЖурналаСообщений.Вставить("УникальныйИдентификатор", СтруктураОтправкиПакета.УникальныйИдентификатор);
	СтруктураЖурналаСообщений.Вставить("НомерСеанса", ПолучитьТекущийСеансИнформационнойБазы().НомерСеанса);	
	СтруктураЖурналаСообщений.Вставить("ВидОперации", СтруктураОтправкиПакета.ВидОперации);
	СтруктураЖурналаСообщений.Вставить("ХранилищеЗапроса", ХранилищеЗапроса);
	СтруктураЖурналаСообщений.Вставить("ХранилищеОтвета", Новый ХранилищеЗначения(ТелоОтвета, МаксимальноеСжатие));
    СтруктураЖурналаСообщений.Вставить("Ресурс", СтруктураОтправкиПакета.АдресРесурса);    
    ...
    Иначе
        ...
        КодОшибкиИнтернетСоединения = 700;
        СтрукутраОтвета.Вставить("ЗаголовкиОтвета", Неопределено);
        СтрукутраОтвета.Вставить("КодСостояния", КодОшибкиИнтернетСоединения);
...
Функция ОписаниеОшибкиВТелоОтвета(ОписаниеОшибки)
